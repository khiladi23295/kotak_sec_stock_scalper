...
df = pd.read_csv("C:/abcd/Some_Path/StockNames.txt", sep=",", header=None,
...
while flag == 1:
    ...
    if now >= my_datetime:
        ...
        for i in instrument_token_list:   # Loop that places the buy orders for both the stocks
            try:
                placed_order_details = client.place_order(order_type="MIS", instrument_token=i, transaction_type="BUY",
                               quantity=1, price=0, disclosed_quantity=0, validity="GFD", variety="REGULAR")  # REGULAR
                time.sleep(1)
        ...
        for i in range(counter):    
            ...
            msg = "Take Profit: "
            exception_logs(msg, take_profit)
